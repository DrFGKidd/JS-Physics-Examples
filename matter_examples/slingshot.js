...
Example.slingshot = function() {
    ...
    Events.on(engine, 'collisionEnd', function(event) {
      ...
      for (i = 0; i<pairs.length; i++) {
        bodyA = pairs[i].bodyA
        bodyB = pairs[i].bodyB
        rel_v = Math.sqrt(Math.pow(bodyA.velocity.x-bodyB.velocity.x,2)+Math.pow(bodyA.velocity.y-bodyB.velocity.y,2))
        if (rel_v>10) {
          console.log(bodyA)
          console.log(bodyB)
          remove_body.push(bodyA)
          remove_body.push(bodyB)
        }
      }
      for (i = 0; i<remove_body.length; i++) {
        Composite.remove(engine.world,remove_body[i]);
        Composite.remove(pyramid,remove_body[i]);
        Composite.remove(pyramid2,remove_body[i]);
      }
    })
