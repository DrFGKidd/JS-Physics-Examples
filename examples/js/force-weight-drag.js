...
var loop = function() {
    if ( ! mouse.isDown) {
        ...
        var Fx = -0.5 * Cd * A * rho * ball.velocity.x * ball.velocity.x * ball.velocity.x / Math.abs(ball.velocity.x)-slingshot.d.x*k_spr;
        var Fy = -0.5 * Cd * A * rho * ball.velocity.y * ball.velocity.y * ball.velocity.y / Math.abs(ball.velocity.y)-slingshot.d.y*k_spr;
        Fx = (isFinite(Fx) ? Fx : 0);
        Fy = (isFinite(Fy) ? Fy : 0);
